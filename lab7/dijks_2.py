...
class Graph:
    ...
    def add_edge(self, frm, to, cost = 0):
        # Check if the 'from' vertex exists in the graph, if not, add it.
        if frm not in self.vert_dict:
            self.add_vertex(frm)
        
        # Check if the 'to' vertex exists in the graph, if not, add it.
        if to not in self.vert_dict:
            self.add_vertex(to)

        # Add the 'to' vertex as a neighbor of the 'from' vertex.
        self.vert_dict[frm].add_neighbor(self.vert_dict[to], cost)

        # add the 'from' vertex as neighbor to the 'to' vertex with the same cost.
...
if __name__ == '__main__':
    ...
    nx.draw_networkx_edges(G, pos, edgelist=SPT_edges, edge_color='r', width=2)
